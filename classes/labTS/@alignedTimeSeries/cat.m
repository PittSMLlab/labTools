...
if ~forceFlag && ~all(strcmp(this.alignmentLabels, ...
    ...
    ME = MException('ATS:cat', ...
        ['Alignment labels mismatch, this check can be ignored by ' ...
        'setting forceFlag = true']);
    throw(ME);
end
...
if dim == 3
    ...
    if ~forceFlag && ~all(strcmp(this.labels, other.labels))
        ME = MException('ATS:cat', ...
            ['Label mismatch, this check can be ignored by setting ' ...
            'forceFlag = true']);
        throw(ME);
    end
    ...
    warning('ATS:catStridesLostEvents', ...
        ['Cat-ting strides of alignedTimeSeries, events are no ' ...
        'longer consecutive.']);
elseif dim == 2 % Cat-ting labels
    ...
    if any(size(this.eventTimes) ~= size(other.eventTimes)) || ...
        ...
        ME = MException('ATS:cat', ...
            ['Trying to cat labels, but event times are ' ...
            'different']);
        throw(ME);
    end
