...
%                 specified by newClass or same as input
%
%   See also: labTimeSeries/split

% Split the data into [t0, t1).
% Args:
%   -t0: time in seconds (relative to trial start) of where to
%        start the split (inclusive) when given NaN, default to
%        start of the trial.
%   -t1: time in seconds (relative to trial start) of where to
%        stop (exclusive). OPTIONAL, when not provided or got
%        NaN, default to end of the trial
...
if nargin < 4
    ...
    metaData = derivedMetaData(labDate.genIDFromClock, ...
        ...
        ['Splice of ' this.metaData.description], ...
        'Auto-generated', this.metaData.refLeg, ...
        this.metaData); % HH removed 'Partial Interval' after
    % labdata.split since 'type' propery
    % was eliminated.
...
for i = 1:length(auxLst)
    % Should try to do this only if the property is not
    % dependent, otherwise, I'm computing things I don't need
    eval(['oldVal = this.' auxLst{i} ';'])
    if isa(oldVal, 'labTimeSeries') && ...
            ~isa(oldVal, 'parameterSeries')
        % no end time point provided, assume spliting from t0 to
        % end of the trial
        ...
        % a flag/fake initial time provided, assuming split from
        % beginning to to t1
        if isnan(t0)
        ...
        % Calling labTS.split (or one of the subclass'
        % implementation)
