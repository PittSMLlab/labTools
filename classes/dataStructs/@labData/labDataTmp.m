...
classdef labData
    ...
    %   HreflexPin - labTS with analog signal with spike once in a 
    %                while representing time when H-reflex 
    ...
    %                    kinetics
    %   estimateSubjectBodyWeight - estimates subject weight from 
    %                               GRF data
    %   process - processes raw data to find angles, events, and 
    %             adaptation parameters. Returns processedTrialData 
    %             object
    ...
    properties % (SetAccess = private)
        ...
        HreflexPin % labTS, sync signal for H-reflex stimulus 
                   % delivery
    end
    ...
    methods
        ...
        function this = labData(metaData, markerData, EMGData, ...
            ...
            elseif isa(beltSpeedSetData, 'labTimeSeries')
                this.beltSpeedSetData = beltSpeedSetData; % Empty or 
                                                          % labels 'L' 
                                                          % and 'R'
            else
                ME = MException('labData:Constructor', ...
            ...
            elseif isa(beltSpeedReadData, 'labTimeSeries')
                this.beltSpeedReadData = beltSpeedReadData; % Empty or 
                                                            % labels 
                                                            % 'L' and 
                                                            % 'R'
            else
            ...
            elseif isa(footSwitches, 'labTimeSeries')
                this.footSwitchData = footSwitches; % Empty or labels 
                                                    % 'L' and 'R'
            else
                ME = MException('labData:Constructor', ...
            ...
            elseif isa(HreflexPin, 'labTimeSeries')
                this.HreflexPin = HreflexPin; % Empty or labels 'L' 
                                              % and 'R'
            else
                ME = MException('labData:Constructor', ...
