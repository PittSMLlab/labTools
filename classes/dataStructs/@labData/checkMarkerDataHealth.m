...
for i = 1:length(ll)
    ...
    if any(aux(i, :))
        warning('labData:checkMarkerDataHealth', ...
            ['Marker ' l ' is missing for ' ...
            num2str(sum(aux) * ts.sampPeriod) ' secs.'])
        for j = 1:3
...
bar(c)
set(gca, 'XTick', 1:numel(ll), 'XTickLabel', ll, ...
...
for i = 1:length(ll)
    if c(i) > .01
        ...
        h(end + 1) = histogram(runs_ones, [.5:1:50], ...
            'DisplayName', ll{i});
        s{end + 1} = ll{i};
    end
end
...
for j = 1:length(ii)
    figure
    plot3(dd(:, 1, ii(j)), dd(:, 2, ii(j)), dd(:, 3, ii(j)), ...
        'o')
    text(dd(:, 1, ii(j)), dd(:, 2, ii(j)), dd(:, 3, ii(j)), ll)
    hold on
    [~, zz] = min(lp(:, ii(j)));
    plot3(dd(zz, 1, ii(j)), dd(zz, 2, ii(j)), ...
        dd(zz, 3, ii(j)), 'ro')
    title(['median likeli = ' num2str(medLike(ii(j))) ...
        ', frame ' num2str(ii(j))])
    pause
end
