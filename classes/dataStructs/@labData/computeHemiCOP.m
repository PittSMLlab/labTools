...
if nargin > 2 && ~isempty(noFilterFlag) && noFilterFlag == 1
    ...
    F = this.getDataAsOTS([side 'F']).medianFilter(5)...
        .substituteNaNs;
    F = F.lowPassFilter(fcut).thresholdByChannel(-100, ...
        [side 'Fz'], 1);
    F = squeeze(F.getOrientedData);
    M = this.getDataAsOTS([side 'M']).medianFilter(5)...
        .substituteNaNs;
...
% t = -aux(:, 3) ./ F(:, 3);
% COP = orientedLabTimeSeries(aux + t .* F, this.Time(1),
% this.sampPeriod,
% orientedLabTimeSeries.addLabelSuffix([side 'COP']),
% this.orientation);
...
aux(:, 3) = 0;
if strcmp(side, 'R')
    % Flipping and offsetting to match reference axis of
    % L-forceplate
    aux(:, 1) = aux(:, 1) - 977.9;
end
...
aux(:, 1) = aux(:, 1) + 25.4;
COP = orientedLabTimeSeries(aux, this.Time(1), ...
    this.sampPeriod, ...
    orientedLabTimeSeries.addLabelSuffix([side 'COP']), ...
    this.orientation);
