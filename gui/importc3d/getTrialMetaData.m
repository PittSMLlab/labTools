...
for cond = sort(info.cond)              % for each exp. condition, ...
    for t = info.trialnums{cond}        % for each trial in condition, ...
        ...
        if datlogExist                  % if data logs exist, ...
            filenameDatlog = ...
                [datalogFolder filesep basename sprintf('%02d',t) '.mat'];
             if ~isfile(filenameDatlog) 
                %datlog folder exists but file name for a given trial doesn't,
                %first try to rename the datlogs to see if that helps.
                fprintf(['\nDatalog folder exists (%s) but could not find ' ...
                        'datalog file for trial: %s. Attempt to rename all the datlog files now.\n'], ...
                        datalogFolder,[basename sprintf('%02d',t)]);
                autoRenameDatlog(info.dir_location,datalogFolder)
                fprintf('\nAuto renaming datlog complete. Check the matchResults to validate if it matches your intuition\n')
             end
            
            try % now try loading data log
                % open data log for this specific condition
                info.datlog{cond} = load(filenameDatlog);
            catch ME
                error(['Datalog folder exists (%s) but could not find ' ...
                    'datalog file for trial: %s even after renaming attempt. Maybe this datlog file was not copied? ' ...
                    'Check your Vicon x1d files and datlog files for this participant.'], ...
                    datalogFolder,[basename sprintf('%02d',t)]);
            end
        end
        fileList{t} = filename;
