    ...
    for t = trials
        ...
        if ~isempty(out.secdir_location)
            filename2 = fullfile(out.secdir_location, ...
                [out.basename sprintf('%02d', t) '.c3d']);
            if ~isfile(filename2)
                % Warn but do not abort: loadTrials handles missing
                % secondary files by filling PC2 channels with NaN,
                % allowing sessions with a subset of missing PC2 files
                % to be processed in a single pass.
                warning('errorProofInfo:missingSecondaryC3D', ...
                    ['The secondary C3D file %s does not exist. PC2 EMG'...
                    ' channels will be filled with NaN for trial ' ...
                    num2str(t) ' by loadTrials.'], filename2);
            end
        end
    end
