...
% Fractional signal-energy threshold above which a sync mismatch
% advisory warning is emitted (1%). Processing always continues.
syncWarnThreshold = 0.01;

% Fractional signal-energy threshold above which a high-mismatch
% warning is emitted indicating poor synchronization quality (5%).
% Processing always continues with the best available alignment so
% that no user-specified trial is discarded or requires user input.
syncHighMismatchThreshold = 0.05;
...
for tr = trialNums                      % for each trial, ...
    ...
    clearvars -except trialMD fileList secFileList info tr ...
        ...
        syncWarnThreshold syncHighMismatchThreshold ...
        accDownsampleFactor syncPlotDuration;
    ...
    if info.EMGs
        ...
        if ~isempty(sync)           % if sync signals present, proceed
            ...
            % Save the sync diagnostic figure for post-hoc review.
            ...
            % Tiered mismatch warnings — the pipeline always proceeds
            % with the best available alignment regardless of mismatch
            % level so that no user-specified trial is ever discarded
            % or requires interactive input. Warnings are written to
            % the diary log for post-hoc review.
            if isnan(E1) || isnan(E2)
                warning('loadTrials:syncMismatchNaN', ...
                    ['[Trial %d] EMG sync mismatch energy could ' ...
                    'not be computed (NaN result). The reference ' ...
                    'signal may have zero energy or matchSignals ' ...
                    'may have failed. Proceeding with best ' ...
                    'available alignment; inspect the saved sync ' ...
                    'figure for trial %d.'], tr, tr);
            elseif E1 > syncHighMismatchThreshold || ...
                    E2 > syncHighMismatchThreshold
                warning('loadTrials:syncMismatchHigh', ...
                    ['[Trial %d] EMG sync mismatch energy is high ' ...
                    '(PC1: %.2f%%, PC2: %.2f%%). At least one PC ' ...
                    'exceeds the high-mismatch threshold of %.0f%%. ' ...
                    'Synchronization quality is poor. Proceeding ' ...
                    'with best available alignment; inspect the ' ...
                    'saved sync figure and verify the data for ' ...
                    'trial %d.'], ...
                    tr, 100*E1, 100*E2, ...
                    100*syncHighMismatchThreshold, tr);
            elseif E1 > syncWarnThreshold || E2 > syncWarnThreshold
                warning('loadTrials:syncMismatchWarn', ...
                    ['[Trial %d] EMG sync mismatch energy is ' ...
                    'elevated (PC1: %.2f%%, PC2: %.2f%%). At least ' ...
                    'one PC exceeds the advisory threshold of %.0f%%. '...
                    'Proceeding with best available alignment; ' ...
                    'inspect the saved sync figure for trial %d.'], ...
                    tr, 100*E1, 100*E2, 100*syncWarnThreshold, tr);
            end
        else
