...
for tr = trialNums                      % for each trial, ...
    ...
    if info.EMGs
        ...
        if ~isempty(sync)           % if sync signals present, proceed
            ...
            if secondFile
                if size(aux, 2) < 2
                    warning('loadTrials:missingPC2SyncChannel', ...
