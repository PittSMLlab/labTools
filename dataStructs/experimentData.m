        ...
        function stridedExp=splitIntoStrides(this)
            if ~this.isStepped && this.isProcessed
                for trial=1:length(this.data)
                    ...
                    aux=trialData.separateIntoStrides('LHS');
                    Lstrides{trial}=aux;
                    aux=trialData.separateIntoStrides('RHS');
                    Rstrides{trial}=aux;
                    else
                        Lstrides{trial}=[];
                        Rstrides{trial}=[];
                ...
                stridedExp=stridedExperimentData(this.metaData,this.subData,Lstrides,Rstrides);
            else
