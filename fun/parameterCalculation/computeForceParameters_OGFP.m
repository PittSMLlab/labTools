...
end_th = 0.2;
divideri = 8;
trialNum = str2double(trialData.rawDataFilename(end-1:end));
...
%                 mini_1st.(Allz{j}) = abs(min(striderFz_OGFP.(Allz{j})(1:end)));
    ...
    if isempty(striderSy_all) || all(striderSy_all==striderSy_all(1)) || isempty(FTO) || isempty(STO)% So if there is some sort of problem with the GRF, set everything to NaN
        ...
        if nanstd(striderSy_all)<0.01 && nanmean(striderSy_all)<0.01 %This is to get rid of places where there is only noise and no data
            ...
            ns_all = find((striderSy_all-LevelofInterest)<0);%1:65
...
%             figure (trialData.condition*10-8)
%             hold on
%             plot(striderSy_all,'b')
...
%             hold off
%             title(['Slow Single Stance Overlapped' '     ' trialData.name])
%             saveas(gcf,['SlowOverlapped_' trialData.name],'png')
             
%             figure (trialData.condition*10-7)
%             plot(str_striderSz.([OGFPz_slowi{i_slow} num2str(i_slow)]),'Color',[1,0,0])
%             for yo=1:8
%                 line([yo*midway_slow yo*midway_slow], get(gca, 'ylim'),'Color',[0 0 0])
%             end
    ...
    if isempty(striderFy_all) || all(striderFy_all==striderFy_all(1)) || isempty(FTO) || isempty(STO)
        ...
        if nanstd(striderFy_all)<0.01 && nanmean(striderFy_all)<0.01 %This is to get rid of places where there is only noise and no data
            ...
            nf_all=find((striderFy_all-LevelofInterest)<0);%1:65
...
% 
%             figure (trialData.condition*10-6)
%             hold on
...
%             hold off
%             title(['Fast Single Stance Overlapped' '     ' trialData.name])
%             saveas(gcf,['FastOverlapped_' trialData.name],'png')
...
%             figure(trialData.condition*10-5)
%             hold on
%             plot(striderSz_OGFP_sum)
%             figure (trialData.condition*10-4)
%             hold on
...
% figure (trialData.condition*10-3)
% hold on
% for i = 1:i_slow
%     %     if i<= i_slow/2
...
% hold off
% %legend(OGFPy_slowi)
% title(['Slow AP' '     ' trialData.name])
% saveas(gcf,['Slow AP_' trialData.name],'png')
% 
% 
% figure (trialData.condition*10-2)
% hold on
% for i = 1:i_slow
%     if i<= i_slow/2
...
% hold off
% %legend(OGFPy_slowi)
% title(['Slow Z' '     ' trialData.name])
% saveas(gcf,['Slow Z_' trialData.name],'png')
% 
% figure (trialData.condition*10-1)
% hold on
% for i = 1:i_fast
...
% end
% hold off
% title(['Fast AP' '     ' trialData.name])
% saveas(gcf,['Fast AP_' trialData.name],'png')
% 
% 
% figure (trialData.condition*10)
% hold on
% for i = 1:i_fast
%     if i<= i_fast/2
...
% hold off
% %legend(OGFPy_fasti)
% title(['Fast Z' '     ' trialData.name])
% saveas(gcf,['Fast Z_' trialData.name],'png')


% figure (trialData.condition*4-3)
% hold on
% plot(striderSy_all)
% hold off
% title('Slow AP')
% saveas(gcf,['Slow AP' num2str(trialData.condition)],'png')
%
% figure (trialData.condition*4-2)
% hold on
% plot(striderSz)
% hold off
% title('Slow Z')
% saveas(gcf,['Slow Z' num2str(trialData.condition)],'png')
%
% figure (trialData.condition*4-1)
% hold on
% plot(striderFy_all)
% hold off
% title('Fast AP')
% saveas(gcf,['Fast AP' num2str(trialData.condition)],'png')
%
% figure (trialData.condition*4)
% hold on
% plot(striderFz)
% hold off
% title('Fast Z')
% saveas(gcf,['Fast Z' num2str(trialData.condition)],'png')
