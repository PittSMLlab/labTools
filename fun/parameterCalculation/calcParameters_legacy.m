function out = calcParameters_legacy(trialData,subData)
in=trialData;
...
for step=1:Nstrides
    ...
    if good(t)
        ...
        if calcSpatial        
            ...
            %frequencies differ %TODO: interpolate kinematic data instead
            %of rounding.
            CF=f_kin/f_events;
...
%             singleStanceSpeedFastAbs(t)=abs(fAnk(indSTO,2)-fAnk(indSHS2,2))/(round((timeSHS2-timeSTO)*f_kin)/f_kin); %Ankle absolute speed: should be exactly belt speed for TM trials, and exactly 0 on OG
            ...
            singleStanceSpeedSlow(t)=nanmedian(f_kin*diff(sAnkPos(indFTO:indFHS)));
            singleStanceSpeedFast(t)=nanmedian(f_kin*diff(fAnkPos(indSTO:indSHS2)));

            sStanceIdxs=indFTO:indFHS;
            fStanceIdxs=indSTO:indSHS2;
            singleStanceSpeedSlowAbs(t)=prctile(f_kin*diff(sAnk(sStanceIdxs,2)),70);
            singleStanceSpeedFastAbs(t)=prctile(f_kin*diff(fAnk(fStanceIdxs,2)),70);
            singleStanceSpeedSlowAbs(t)=prctile(f_kin*diff(sToe(sStanceIdxs,2)),70);
            singleStanceSpeedFastAbs(t)=prctile(f_kin*diff(fToe(fStanceIdxs,2)),70);
