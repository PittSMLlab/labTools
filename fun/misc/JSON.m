    ...
    methods (Access = public)
        ...
        function array = getArray(this)
            ...
            while ~strcmp(value, ']')
                ...
                elseif strcmp(value, ']')
                    ...
                    error(['JSON parser requires commas between ' ...
                        'array elements']);
                end
            end
        ...
        function obj = getObject(this)
            ...
            while ~strcmp(value, '}')
                ...
                fieldname = value;
                % make sure it is a valid structure field name
                fieldname = strrep(fieldname, ':', '_');
                fieldname = strrep(fieldname, '-', '_');
                % fix for field names that start with numbers (Guy Ziv)
                fieldname = regexprep(fieldname, '(^\d)', 's$1');

                % check for colon
                value = this.getValue();
                if ~strcmp(value, ':')
                    error(['JSON parser requires colons between ' ...
        ...
        function [token, tokenType] = getNextToken(this)
            ...
            function match(this, str)
                % consume exactly str at current position or error
                n     = length(str);
                ...
                if strcmp(str, found)
                    ...
                    error('The JSON parser expected "%s" but found %s', ...
                        str, found)
                end
            end
            ...
            function string = getString(this)
                ...
                string     = str(first:(last-1));   % string without quotes
            ...
            function number = getNumber(this)
                ...
                function tf = isDigit(aChar)
                    tf = aChar > 47 && aChar < 58;  % ASCII '0'-'9'
                end
