...
% pause(2);
% processAndFillMarkerGapsTrial(trialOld, viconOld);
% pause(3);
% 
...
%     
%     if isempty(trajOld) || isempty(trajNew) || all(all(isnan(trajOld))) || all(all(isnan(trajNew)))
%         warning('Skipping plot for %s due to missing data.', marker);
%         continue;
%     end
%     
...
%     time = (1:nFrames) / 100;
%     
%     figure('Name', ['Overlaid Trajectories - ' marker], 'NumberTitle', 'off');
%     tl = tiledlayout(3,1);
%     
%     for dim = 1:3
%         nexttile;
